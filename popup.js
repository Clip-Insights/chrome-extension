...
// const notesDatabase = new YouTubeNotesDatabase();   // creating database to store notes and screenshots
// console.log("inside popup created database");
...
document.getElementById('clipinsights__screenshotBtn').addEventListener('click', async () => {
  takeScreenshot();
});
...
document.getElementById('clipinsights__noteInput').addEventListener('keydown', (event) => {
  if (event.key === 'Enter') {
...
document.getElementById('clipinsights__clearBtn').addEventListener('click', clearAllScreenshotsAndNotes);
document.getElementById('clipinsights__saveBtn').addEventListener('click', saveAsPDF);
...
function formatKeypoints(bulletString) {
  ...
  // Return the formatted keypoints wrapped in an unordered list
...
document.getElementById('clipinsights__summaryBtn').addEventListener('click', async () => {
...
function addScreenshotToPopup(screenshotUrl, time) {
  const screenshotNoteContainer = document.getElementById('clipinsights__screenshotNoteContainer');
...
function addNoteToPopup(note, time) {
  ...
  const noteInput = document.getElementById("clipinsights__noteInput");
...
function restoreScreenshotsAndNotes() {
  ...
  if (savedSummary) {
    document.getElementById('clipinsights__summary').innerHTML = formatSummaryToHTML(savedSummary);
    document.getElementById('clipinsights__keypoints').innerHTML = formatKeypoints(savedKeypoints);
  }
}
...
function clearAllScreenshotsAndNotes() {
  ...
  document.getElementById('clipinsights__keypoints').innerHTML = '' // Clear the summary from the UI

  // Clear from localStorage
  localStorage.removeItem('screenshots');
  localStorage.removeItem('notes');
  localStorage.removeItem('summary');
  localStorage.removeItem('keypoints') // Clear the summary from localStorage
}
...
async function saveAsPDF() {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
    ...
    if (savedKeypoints) {
      ...
      const keypoints = savedKeypoints.split('\n'); // Split keypoints by newline
      doc.setFontSize(12); // Slightly smaller font for keypoints
  
      keypoints.forEach((bullet) => {
          if (y + 10 > pageHeight) {
              doc.addPage();
              y = 10;
          }
...
const backBtn = document.getElementById('clipinsights__backBtn');
const mainContent = document.getElementById('clipinsights__mainContent');
const loginContainer = document.getElementById('clipinsights__loginContainer');
...
const submitLogin = document.getElementById('clipinsights__submitLogin');
...
const chatBtn = document.getElementById("clipinsights__chatBtn");
const chatContainer = document.getElementById("clipinsights__chatContainer");
const closeChat = document.getElementById("clipinsights__closeChat");
...
const sendChatBtn = document.getElementById("clipinsights__sendChatBtn");
...
//         // Save the new title to localStorage or your preferred storage
//         localStorage.setItem('customVideoTitle', newTitle);
//       }
...
// Function to initialize title display and editing
// function initializeTitle() {
//   chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
//     if (!tabs[0].url.includes('youtube.com/watch')) return;
